...
#endif
{
    addParameter(new juce::AudioParameterFloat("rate", "Rate", 0.0f, 20.0f, 5.0f)); // rate is in Hz
    addParameter(new juce::AudioParameterFloat("depth", "Depth", 0.0f, 1.0f, 0.5f));
...
void TremoloOSHEAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float rate = parameters[0]->getValue();
    float depth = parameters[1]->getValue();
    float gain = parameters[2]->getValue();
    
    double sampleRate = this->getSampleRate(); //https://forum.juce.com/t/sample-rate-in-pluginprocessor-processblock-function/54671/2
    ...
    position += 1;
    if (position >= (6.2831853 / w)) // Calculate number of samples in one LFO period with 2pi/w and see if position is beyond that
        position = 0;
}
