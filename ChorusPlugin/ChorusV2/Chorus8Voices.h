        ...
        for (int channel = 0; channel < totalNumInputChannels; ++channel)
        {
            ...
            if (channel == 0)
            {
                for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
                {
                    ...
                    lfoValue = abs(chnl1LFO.processSample(0.0f));//lfoValue = chnl1LFO.processSample(0.0f);
            ...
            else // Handles the second channel for stereo input but doesn't run for mono input
            {
                for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
                {
                    ...
                    lfoValue = abs(chnl2LFO.processSample(0.0f));//lfoValue = chnl2LFO.processSample(0.0f);
