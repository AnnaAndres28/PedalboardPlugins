        ...
        for (int channel = 0; channel < totalNumInputChannels; ++channel)
        {
            ...
            if (channel == 0)
            {
                for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
                {
                    ...
                    wetSample = chnl1delay.popSample(channel, delayInSamples, true) * feedbackFloat;
            ...
            else
            {
                for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
                {
                    ...
                    drySample = channelData[sample];
                    wetSample = chnl2delay.popSample(channel, delayInSamples, true) * feedbackFloat;
                    wetSample = (wetSample * ((depthFloat/5) * lfoValue + (1.0f - (depthFloat/5)))); // Amplitude Modulation
...
private:
    ...
    juce::dsp::DelayLine<float> chnl1delay;
    juce::dsp::DelayLine<float> chnl2delay;
